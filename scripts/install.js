...
function getLogStream(defaultStream) {
  ...
  throw new Error(
    `Incorrect GSOC2CLI_LOG_STREAM env variable. Possible values: 'stdout' | 'stderr'`
  );
}
...
function shouldRenderProgressBar() {
  ...
  const silentEnv = process.env.GSOC2CLI_NO_PROGRESS_BAR;
...
function getDownloadUrl(platform, arch) {
  const releasesUrl = `${CDN_URL}/${pkgInfo.version}/gsoc2-cli`;
  let archString = '';
  switch (arch) {
    case 'x64':
      archString = 'x86_64';
...
async function downloadBinary() {
  ...
  if (process.env.GSOC2CLI_USE_LOCAL === '1') {
    try {
      const binPaths = which.sync('gsoc2-cli', { all: true });
      ...
      throw new Error(
        ...
        'Make sure that `gsoc2-cli` executable is available in your $PATH or disable GSOC2CLI_USE_LOCAL env variable.'
      );
    }
  }
  ...
  try {
    ...
    let errorMsg = `Unable to download gsoc2-cli binary from ${downloadUrl}.\nError message: ${error.message}`;
    if (error.code) {
  ...
  if (process.env.GSOC2CLI_SKIP_CHECKSUM_VALIDATION !== '1') {
...
async function checkVersion() {
  ...
  const version = output.replace('gsoc2-cli ', '').trim();
  const expected = pkgInfo.version;
  if (version !== expected) {
    throw new Error(`Unexpected gsoc2-cli version "${version}", expected "${expected}"`);
  }
}

if (process.env.GSOC2CLI_SKIP_DOWNLOAD === '1') {
  logger.log(`Skipping download because GSOC2CLI_SKIP_DOWNLOAD=1 detected.`);
  process.exit(0);
}
...
try {
  ...
  logger.log(
    `Gsoc2 CLI failed to locate the "${distributionPackageName}" package after installation!

This can happen if you use an option to disable optional dependencies during installation, like "--no-optional", "--ignore-optional", or "--omit=optional". Gsoc2 CLI uses the "optionalDependencies" package.json feature to install the correct binary for your platform and operating system. This post-install script will now try to work around this by manually downloading the Gsoc2 CLI binary from the Gsoc2 CDN. If this fails, you need to remove the "--no-optional", "--ignore-optional", and "--omit=optional" flags for Gsoc2 CLI to work.`
  );
