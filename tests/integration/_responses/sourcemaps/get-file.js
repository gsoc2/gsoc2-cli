...
function r(t) {
  ...
  ['debug', 'info', 'warn', 'error', 'log', 'assert'].forEach((t) => {
    t in s.console &&
      e[t].__gsoc2_original__ &&
      ((n[t] = e[t]), (e[t] = e[t].__gsoc2_original__));
  });
  const r = t();
  return (
...
(new (class {
  ...
  log(...t) {
    this.enabled || r(() => this._global.console.log(`Gsoc2 ${this._name} [Log]: ${t.join(' ')}`));
  }
  warn(...t) {
    this.enabled ||
      r(() => this._global.console.warn(`Gsoc2 ${this._name} [Warn]: ${t.join(' ')}`));
  }
  error(...t) {
    this.enabled ||
      r(() => this._global.console.error(`Gsoc2 ${this._name} [Error]: ${t.join(' ')}`));
  }
})().enabled = !0),
  i();
