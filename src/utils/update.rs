...
use crate::api::{Api, Gsoc2CliRelease};
...
fn rename_exe(exe: &Path, downloaded_path: &Path, elevate: bool) -> Result<()> {
    ...
    let tmp = env::temp_dir().join(".gsoc2-cli.tmp");
...
impl LastUpdateCheck {
    pub fn update_for_info(&mut self, ui: &Gsoc2CliUpdateInfo) {
...
pub struct Gsoc2CliUpdateInfo {
    latest_release: Option<Gsoc2CliRelease>,
}

impl Gsoc2CliUpdateInfo {
    ...
    pub fn download(&self) -> Result<()> {
        ...
        let tmp_path = if elevate {
            env::temp_dir().join(".gsoc2-cli.part")
        } else {
            exe.parent().unwrap().join(".gsoc2-cli.part")
        };
...
pub fn get_latest_gsoc2cli_release() -> Result<Gsoc2CliUpdateInfo> {
    let api = Api::current();
    Ok(Gsoc2CliUpdateInfo {
        latest_release: if let Ok(release) = api.get_latest_gsoc2cli_release() {
            release
        } else {
            None
        },
...
pub fn can_update_gsoc2cli() -> bool {
...
pub fn assert_updatable() -> Result<()> {
    if is_homebrew_install() {
        ...
        println!("Please use homebrew to update gsoc2-cli:");
        println!();
        println!("{} brew upgrade gsoc2-cli", style("$").dim());
        return Err(QuietExit(1).into());
    } else if is_npm_install() {
        println!("This installation of gsoc2-cli is managed through npm/yarn");
        println!("Please use npm/yarn to update gsoc2-cli, using one of the following commands:");
        println!("  yarn upgrade @gsoc2/cli");
        println!("  yarn global upgrade @gsoc2/cli");
        println!("  npm update @gsoc2/cli");
        println!("  npm update -g @gsoc2/cli");
        return Err(QuietExit(1).into());
    }
    Ok(())
}
...
fn update_nagger_impl() -> Result<()> {
    ...
    if check.should_run_check() {
        ...
        let ui = get_latest_gsoc2cli_release()?;
    ...
    if check.is_outdated() {
        ...
        eprintln!(
            "{}",
            style(format!(
                "gsoc2-cli update to {} is available!",
