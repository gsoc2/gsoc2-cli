...
impl<'a> MayDetach<'a> {
    ...
    pub fn may_detach(&mut self) -> Result<bool> {
        ...
        show_notification("Gsoc2", &format!("{} starting", self.task_name))?;
    ...
    fn show_critical_info(&self) -> Result<bool> {
        show_critical_info(
            ...
            "The Gsoc2 build step failed while running in the background. \
    ...
    fn show_done(&self) -> Result<()> {
        if self.is_detached() {
            show_notification("Gsoc2", &format!("{} finished", self.task_name))?;
        }
        Ok(())
    }
}
