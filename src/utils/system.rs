...
{
    // See: https://github.com/gsoc2/gsoc2-cli/pull/1104
...
pub fn print_error(err: &Error) {
    ...
    if Config::current_opt().map_or(true, |config| {
        ...
        eprintln!();
        eprintln!("{}", style("Add --log-level=[info|debug] or export GSOC2_LOG_LEVEL=[info|debug] to see more output.").dim());
        eprintln!(
            "{}",
...
/// Indicates that gsoc2-cli should quit without printing anything.
#[derive(thiserror::Error, Debug)]
#[error("gsoc2-cli exit with {0}")]
pub struct QuietExit(pub i32);

/// Loads a .env file
pub fn load_dotenv() {
    if env::var("GSOC2_LOAD_DOTENV")
        .map(|x| x.as_str() == "1")
        .unwrap_or(true)
    {
