...
pub fn attach_logfile(event: &mut Event<'_>, logfile: &str, with_component: bool) -> Result<()> {
    ...
    // gsoc2 currently requires timestamps for breadcrumbs at all times.
...
/// Returns SDK information for gsoc2-cli.
...
/// Executes the callback with an isolate gsoc2 client on an empty isolate scope.
///
...
pub fn with_gsoc2_client<F, R>(dsn: Dsn, callback: F) -> R
where
    F: FnOnce(&Client) -> R,
{
