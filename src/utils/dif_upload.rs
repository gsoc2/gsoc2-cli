...
impl<'data> ChunkedDifMatch<'data> {
    ...
    // gsoc2-cli versions keep working with ancient versions of gsoc2 by not
    // triggering this protocol bug in most common situations.
    // See: https://github.com/gsoc2/gsoc2-cli/issues/980
    // See: https://github.com/gsoc2/gsoc2-cli/issues/1056
...
/// companion files or metadata files needed to process the DIFs in gsoc2-cli,
/// or later even on Gsoc2.
#[derive(Debug)]
enum DifSource<'a> {
...
fn collect_object_dif<'a>(
    ...
    for object in archive.objects() {
        ...
        // Gsoc2 requires object files to have one during upload.
        ...
        if chunks_missing {
            return Err(format_err!(
                "Some uploaded files are now missing on the server. Please retry by running \
                 `gsoc2-cli upload-dif` again. If this problem persists, please report a bug.",
            ));
        }
...
impl DifUpload {
    ...
    fn validate_capabilities(&mut self) {
        ...
        {
            warn!("Source uploads are not supported by the configured Gsoc2 server");
        ...
        if (self.formats.contains(&DifFormat::Object(FileFormat::Pdb))
            ...
            && !self.pdbs_allowed
        {
            warn!("PDBs and PEs are not supported by the configured Gsoc2 server");
        ...
        {
            warn!("Portable PDBs are not supported by the configured Gsoc2 server");
            // This is validated additionally in .valid_format()
        }
        ...
        {
            warn!("BCSymbolMaps are not supported by the configured Gsoc2 server");
        }

        if self.upload_il2cpp_mappings && !self.il2cpp_mappings_allowed {
            warn!("il2cpp line mappings are not supported by the configured Gsoc2 server");
        }
    }
