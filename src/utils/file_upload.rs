...
/// Old versions of Gsoc2 cannot assemble artifact bundles straight away, they require
...
pub fn initialize_legacy_release_upload(context: &UploadContext) -> Result<()> {
    // if the remote gsoc2 service supports artifact bundles, we don't
        ...
        eprintln!(
            ...
            style(
                "warning: no project specified. \
                    ...
                    this is what you wanted. Future versions of gsoc2 will \
    ...
    if let Some(version) = context.release {
        ...
        bail!("This version of Gsoc2 does not support artifact bundles. A release slug is required (provide with --release)");
    }
    Ok(())
}
...
pub struct SourceFile {
    ...
    ///
    /// Headers that `gsoc2-cli` knows about are
...
fn upload_files_chunked(
    ...
    if !chunks.is_empty() {
        ...
        println!("{} Uploaded files to Gsoc2", style(">").dim());
    } else {
        println!(
