//! This module implements the API access to the Gsoc2 API as well
...
use gsoc2::protocol::{Exception, Values};
use gsoc2::types::Dsn;
...
pub struct Gsoc2Error {
    status: u32,
    detail: Option<String>,
    extra: Option<serde_json::Value>,
}

impl fmt::Display for Gsoc2Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let detail = self.detail.as_deref().unwrap_or("");
        write!(
            f,
            "gsoc2 reported an error: {} (http status: {})",
            if detail.is_empty() {
                match self.status {
                    400 => "bad request",
                    401 => "unauthorized",
...
#[error("project was renamed to '{0}'\nPlease use this slug in your .gsoc2clirc file, gsoc2.properties file or in the CLI --project parameter")]
...
pub enum ApiErrorKind {
    ...
    ReleaseNotFound,
    #[error("chunk upload endpoint not supported by gsoc2 server")]
...
impl Api {
    ...
    /// Finds the latest release for gsoc2-cli on GitHub.
    pub fn get_latest_gsoc2cli_release(&self) -> ApiResult<Option<Gsoc2CliRelease>> {
        ...
        let ref_name = format!("gsoc2-cli-{}-{}{}", capitalize_string(PLATFORM), arch, EXT);
        ...
        if resp.status() == 200 {
            ...
            for (filename, download_url) in info.file_urls {
                ...
                if filename == ref_name {
                    return Ok(Some(Gsoc2CliRelease {
        ...
        T: AsRef<(Digest, &'data [u8])> + 'data,
    {
        ...
        // to be a Gsoc2-compatible endpoint, we force the Authorization header at
        // this point.
    ...
    /// Looks up an event, which was already processed by Gsoc2 and returns it.
    /// If it does not exist `None` will be returned.
    pub fn get_event(
        &self,
        org: &str,
    ...
    fn create(
        ...
        match pipeline_env {
            Some(env) => {
                debug!("pipeline: {}", env);
                headers
                    .append(&format!("User-Agent: gsoc2-cli/{VERSION} {env}"))
                    .ok();
            }
            None => {
                headers
                    .append(&format!("User-Agent: gsoc2-cli/{VERSION}"))
                    .ok();
            }
        }
...
impl ApiResponse {
    ...
    pub fn into_result(self) -> ApiResult<Self> {
        if let Some(ref body) = self.body {
            ...
            if let Ok(dir) = env::var("GSOC2_DUMP_RESPONSES") {
...
/// Information about gsoc2 CLI releases
pub struct Gsoc2CliRelease {
    pub version: String,
