...
use regex::Regex;
use gsoc2::protocol::{Event, Exception, Frame, Stacktrace, User, Value};
use username::get_user_name;
use uuid::Uuid;

use crate::config::Config;
use crate::utils::event::{attach_logfile, get_sdk_info, with_gsoc2_client};
    ...
    command
        ...
        .arg(
            Arg::new("cli")
                .long("cli")
                .value_name("CMD")
                .help("Explicitly set/override the gsoc2-cli command"),
        )
        .arg(
            Arg::new("send_event")
                .long("send-event")
...
fn send_event(
    ...
    if let Ok(f) = fs::File::open(traceback) {
        ...
        for line in f.lines() {
            ...
            if let Some(cap) = FRAME_RE.captures(&line) {
                match &cap[1] {
                    "_gsoc2_err_trap" | "_gsoc2_exit_trap" | "_gsoc2_traceback" => continue,
                    _ => {}
                }
                frames.push(Frame {
    ...
    let id = with_gsoc2_client(config.get_dsn()?, |c| c.capture_event(event, None));
    println!("{id}");
...
pub fn execute(matches: &ArgMatches) -> Result<()> {
    ...
    let log = path.join(format!(".gsoc2-{}.out", Uuid::new_v4().as_hyphenated()));
    ...
    let mut script = BASH_SCRIPT
        .replace(
            "___GSOC2_TRACEBACK_FILE___",
            &traceback.display().to_string(),
        )
        .replace("___GSOC2_LOG_FILE___", &log.display().to_string());
    ...
    script = match release {
        ...
        None => script.replace(" ___GSOC2_RELEASE___", ""),
    };
    ...
    if matches.get_flag("no_environ") {
        script = script.replace("___GSOC2_NO_ENVIRON___", "--no-environ");
    } else {
        script = script.replace("___GSOC2_NO_ENVIRON___", "");
    }
