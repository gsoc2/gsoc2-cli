...
use log::{debug, warn};
use gsoc2::protocol::{Event, Level, LogEntry, User};
use gsoc2::types::{Dsn, Uuid};
use gsoc2::Envelope;
...
pub fn make_command(command: Command) -> Command {
    command.about("Send a manual event to Gsoc2.")
        .long_about(
            "Send a manual event to Gsoc2.{n}{n}\
             This command will validate input parameters and attempt to send an event to \
             Gsoc2. Due to network errors, rate limits or sampling the event is not guaranteed to \
             actually arrive. Check debug output for transmission errors by passing --log-level=\
             debug or setting `GSOC2_LOG_LEVEL=debug`.",
        )
        .arg(
            Arg::new("path")
                .value_name("PATH")
...
fn send_raw_event(event: Event<'static>, dsn: Dsn) -> Uuid {
    debug!("{:?}", event);
    with_gsoc2_client(dsn, |c| c.capture_event(event, None))
}
