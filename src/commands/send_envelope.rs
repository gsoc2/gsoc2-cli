...
pub fn make_command(command: Command) -> Command {
    command
        .about("Send a stored envelope to Gsoc2.")
        .long_about(
            "Send a stored envelope to Gsoc2.{n}{n}\
             This command will validate and attempt to send an envelope to Gsoc2. \
...
pub fn send_raw_envelope(envelope: Envelope, dsn: Dsn) {
    debug!("{:?}", envelope);
    with_gsoc2_client(dsn, |c| c.send_envelope(envelope));
}
