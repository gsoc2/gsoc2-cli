...
use crate::utils::update::{assert_updatable, can_update_gsoc2cli, get_latest_gsoc2cli_release};

pub fn make_command(command: Command) -> Command {
    let command = command.about("Update the gsoc2-cli executable.").arg(
        Arg::new("force")
            .long("force")
            .short('f')
...
pub fn execute(matches: &ArgMatches) -> Result<()> {
    ...
    env::set_var("GSOC2_DISABLE_UPDATE_CHECK", "true");

    // Aborts with an error if this installation is not updatable.
    assert_updatable()?;

    let exe = env::current_exe()?;
    let update = get_latest_gsoc2cli_release()?;
