...
use gsoc2::protocol::{MonitorCheckIn, MonitorCheckInStatus, MonitorConfig, MonitorSchedule};
use gsoc2::types::Dsn;

use crate::api::{Api, ApiCreateMonitorCheckIn, ApiMonitorCheckInStatus, ApiUpdateMonitorCheckIn};
use crate::config::Config;
use crate::utils::event::with_gsoc2_client;
    ...
    command
        ...
        .arg(
            Arg::new("allow_failure")
                ...
                .help("Run provided command even when Gsoc2 reports an error.")
                .hide(true),
        )
        .arg(
            Arg::new("args")
...
fn run_program(args: Vec<&String>, monitor_slug: &str) -> (bool, Option<i32>, Duration) {
    ...
    p.args(&args[1..]);
    p.env("GSOC2_MONITOR_SLUG", monitor_slug);
...
fn dsn_execute(
    ...
    with_gsoc2_client(dsn.clone(), |c| c.send_envelope(open_checkin.into()));
    ...
    with_gsoc2_client(dsn, |c| c.send_envelope(close_checkin.into()));
