        ...
        .arg(
            Arg::new("force")
                ...
                .help(
                    ...
                    produce any information that the gsoc2 build tool could pick up on.",
                ),
        )
        .arg(
            Arg::new("allow_fetch")
...
pub fn execute(matches: &ArgMatches) -> Result<()> {
    ...
    MayDetach::wrap("React native symbol handling", |md| {
        ...
        //
        // this invokes via an indirection of gsoc2-cli our wrap_call() below.
        ...
        // invokes.  Because we export __GSOC2_RN_WRAP_XCODE_CALL=1, the main
        // gsoc2-cli script will invoke our wrap_call() function below.
        //
        ...
        // upload process.
            ...
            command
                ...
                .env("GSOC2_RN_REAL_NODE_BINARY", &node)
                .env(
                    "GSOC2_RN_SOURCEMAP_REPORT",
                    report_file.path().to_str().unwrap(),
                )
                .env("__GSOC2_RN_WRAP_XCODE_CALL", "1");

            if is_hermes_enabled(&hermesc) {
                command
                    .env("HERMES_CLI_PATH", env::current_exe()?.to_str().unwrap())
                    .env("GSOC2_RN_REAL_HERMES_CLI_PATH", &hermesc);
            }
        ...
        // sourcemaps.
        println!("Processing react-native sourcemaps for Gsoc2 upload.");
        ...
        let dist_from_env = env::var("GSOC2_DIST");
        let release_from_env = env::var("GSOC2_RELEASE");
...
pub fn wrap_call() -> Result<()> {
    ...
    let bundle_command = env::var("GSOC2_RN_BUNDLE_COMMAND");
    let compose_source_maps_path = env::var("COMPOSE_SOURCEMAP_PATH");
    let no_debug_id = env::var("GSOC2_RN_NO_DEBUG_ID").unwrap_or("0".to_string()) == "1";

    let report_file_path = env::var("GSOC2_RN_SOURCEMAP_REPORT").unwrap();
    ...
    let executable = if execute_hermes_compiler {
        env::var("GSOC2_RN_REAL_HERMES_CLI_PATH").unwrap()
    } else {
        env::var("GSOC2_RN_REAL_NODE_BINARY").unwrap()
    };
