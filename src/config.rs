...
/// Represents the `gsoc2-cli` config.
pub struct Config {
    filename: PathBuf,
    process_bound: bool,
    ini: Ini,
...
impl Config {
    ...
    pub fn get_base_url(&self) -> Result<&str> {
        ...
        if !is_absolute_url(base) {
            bail!("bad gsoc2 url: unknown scheme ({})", base);
        }
        if base.matches('/').count() != 2 {
            bail!("bad gsoc2 url: not on URL root ({})", base);
        }
        Ok(base)
    }
    ...
    pub fn get_release(&self, matches: &ArgMatches) -> Result<String> {
        matches
            ...
            .cloned()
            .or_else(|| env::var("GSOC2_RELEASE").ok())
    ...
    pub fn get_pipeline_env(&self) -> Option<String> {
        env::var("GSOC2_PIPELINE").ok().or_else(|| {
            self.ini
    ...
    pub fn get_org_and_project_defaults(&self) -> (Option<String>, Option<String>) {
        (
            env::var("GSOC2_ORG").ok().or_else(|| {
                self.ini
                    .get_from(Some("defaults"), "org")
                    .map(str::to_owned)
            }),
            env::var("GSOC2_PROJECT").ok().or_else(|| {
                self.ini
                    .get_from(Some("defaults"), "project")
                    .map(str::to_owned)
            }),
    ...
    pub fn get_max_retry_count(&self) -> Result<u32> {
        if env::var_os("GSOC2_HTTP_MAX_RETRIES").is_some() {
            Ok(env::var("GSOC2_HTTP_MAX_RETRIES")?.parse()?)
    ...
    pub fn get_dsn(&self) -> Result<Dsn> {
        if let Ok(val) = env::var("GSOC2_DSN") {
            Ok(val.parse()?)
    ...
    pub fn get_environment(&self) -> Option<String> {
        if env::var_os("GSOC2_ENVIRONMENT").is_some() {
            env::var("GSOC2_ENVIRONMENT").ok()
        } else {
            self.ini
    ...
    pub fn disable_update_nagger(&self) -> bool {
        if let Ok(var) = env::var("GSOC2_DISABLE_UPDATE_CHECK") {
            &var == "1" || &var == "true"
    ...
    pub fn get_allow_failure(&self, matches: &ArgMatches) -> bool {
        matches.get_flag("allow_failure")
            || if let Ok(var) = env::var("GSOC2_ALLOW_FAILURE") {
                &var == "1" || &var == "true"
            } else {
                false
            }
...
fn find_project_config_file() -> Option<PathBuf> {
    env::current_dir().ok().and_then(|mut path| loop {
        ...
        path.set_file_name("gsoc2cli.ini");
        if path.exists() {
            return Some(path);
        }
        path.pop();
...
fn load_cli_config() -> Result<(PathBuf, Ini)> {
    ...
    if let Ok(prop_path) = env::var("GSOC2_PROPERTIES") {
        match fs::File::open(&prop_path) {
            Ok(f) => {
                ...
                info!(
                    "Loaded file referenced by GSOC2_PROPERTIES ({})",
                    &prop_path
                );
            ...
            Err(err) => {
                if err.kind() != io::ErrorKind::NotFound {
                    return Err(Error::from(err).context(format!(
                        "Failed to load file referenced by GSOC2_PROPERTIES ({})",
                        &prop_path
                    )));
                } else {
                    warn!(
                        "Failed to find file referenced by GSOC2_PROPERTIES ({})",
                        &prop_path
                    );
                }
            }
...
fn get_default_auth(ini: &Ini) -> Option<Auth> {
    if let Ok(val) = env::var("GSOC2_AUTH_TOKEN") {
        Some(Auth::Token(val.into()))
    } else if let Ok(val) = env::var("GSOC2_API_KEY") {
        Some(Auth::Key(val))
...
fn get_default_url(ini: &Ini) -> String {
    if let Ok(val) = env::var("GSOC2_URL") {
        val
...
fn get_default_log_level(ini: &Ini) -> log::LevelFilter {
    if let Ok(level_str) = env::var("GSOC2_LOG_LEVEL") {
...
fn get_default_vcs_remote(ini: &Ini) -> String {
    if let Ok(remote) = env::var("GSOC2_VCS_REMOTE") {
        remote
